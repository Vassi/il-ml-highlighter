...
class ILMLFoldingProvider {
    ...
    closeRegionOfType(regionStack, ranges, type, endLine) {
    ...
    closeTemplateRegion(regionStack, ranges, templateName, endLine) {
        for (let j = regionStack.length - 1; j >= 0; j--) {
    ...
    findRegionOfType(regionStack, type) {
    ...
    closeSubBlocksAtIndentation(regionStack, ranges, currentIndentation, beforeLine) {
        while (regionStack.length > 0) {
            const topRegion = regionStack[regionStack.length - 1];
            if (topRegion.type !== 'sub-block')
                break;
    ...
    closeRemainingRegions(regionStack, ranges, lastLine) {
        while (regionStack.length > 0) {
            const region = regionStack.pop();
    ...
    findLastNonEmptyLine(beforeLine) {
